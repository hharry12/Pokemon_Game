class Pokemon:
    ...
    # Method for reviving a Pokemon ***Not currently used***
    def revive(self):
    ...
    def attack(self, victim, trainer_name):

        if self.knocked_out == False:

            # Determine type difference - atm only works for fire grass and water
            if ((self.type == "water" and victim.type == "fire") or 
            (self.type == "fire" and victim.type == "grass") or 
            (self.type == "grass" and victim.type == "water")):
             multiplier = 2
            else:
             multiplier = 0.5 
            damage = self.level*multiplier
            #call lose_health method
            result = self.name + " attacks " + trainer_name + "'s " + victim.name + ". " 
